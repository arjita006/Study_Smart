...
using System.IO;
public partial class account_tutor_question : System.Web.UI.Page
{
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        string category = "Board Question Exam";
        ...
        try
        {
            if (!string.IsNullOrEmpty(name.Trim()) && !string.IsNullOrEmpty(year.Trim()) && !string.IsNullOrEmpty(c.Trim()) && !string.IsNullOrEmpty(l.Trim()))
            {
                if (FileUpload1.HasFile && FileUpload2.HasFile)
                {
                    ...
                    if (check_ext(filename) && check_image(filename1))
                    {
                        ...
                        if (a < 512 && b < 512)
                        {
                            string path = "~/account/acc_file/" + filename;
                            FileUpload1.SaveAs(Server.MapPath(path));
                            FileUpload2.SaveAs(Server.MapPath("~/") + filename1);
                            string sql = "exec ques_board @userid=" + tutor_id + ",@user_email='" + email + "', @papername='" + name + "',@paperfile='" + path + "', @paperimage=@image2 , @paperyear='" + year + "',@langid=" + lang + ", @classid=" + cl + ",@classname='" + c + "',@langname='" + l + "', @action=4";
                            string sql1 = "insert into tutor_upload(user_email,category,file_name) values('" + email + "','" + category + "','" + filename + "')";
                            int q = al.insert(sql1);
                        ...
                        else if (b > 512)
                        {
                            ...
                            lblmsg1.Text = "Image should not be more than 512 Kb";
                        }
            ...
            {
                lblmssg.Text = "Please fill in all fields";
                lblmssg.ForeColor = Color.Red;
            }
        }
        catch (Exception ex)
